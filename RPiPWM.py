...
_parrot_ms = 230    # коэффициент преобразования 205 попугаев ~ 1 мс
_min = 183  # 1 мс (~ 4096/20)
_max = 503  # 2 мс (~ 4096*2/20)
...
_wideMin = 114     # 0.5 мс (~ _min*0.5)   # тестово
_wideMax = 572    # 2.5 мс (~_max*1.25)
...
class PwmBase:
    def __init__(self, channel, mode, extended=False):
        ...
        if not _pwmIsInited:    # если микросхема еще не была инициализирована
            ...
            time.sleep(0.005)
            self._SetPwmFreq(55)    # устанавливаем частоту сигнала 50 Гц
